...
export default function App() {
  ...
  const doIssuePrep = (workerId, matId, qty, typeId) => {
    ...
    openConfirm('Видача на заготовку',
      ...
      async () => {
        closeModal()
        const item = {
          id: uid(), workerId: worker.id, workerName: worker.name, typeId: type.id,
  ...
  // ════════════════════════════════════════════════════════
  const wrap = (children) =>
    <div style={{ padding: '12px 12px calc(80px + env(safe-area-inset-bottom, 20px))', maxWidth: 700, margin: '0 auto' }}>{children}</div>
  ...
  const PageRepair = () => {
    ...
    const returnAll = (r) => openConfirm('Повернути всі матеріали?', 'Повернуться на склад.', async () => {
      ...
      if (type) setBatteryTypes(prev => prev.map(t => t.id !== r.typeId ? t : {
        ...t, materials: t.materials.map(m => {
  ...
  //  LAYOUT
  // ════════════════════════════════════════════════════════
  const NAV = [['prod', '⚙', 'ВИР.'], ['stock', '📦', 'СКЛАД'], ['repair', '🔧', 'РЕМОНТ'], ['shopping', '🛒', 'ЗАКУПІВЛЯ'], ['workers', '👷', 'КОМАНДА'], ['tools', '🛠', 'ІНСТР.'], ['log', '📋', 'ЖУРНАЛ']]
  const PAGES = { prod: <PageProd />, stock: <PageStock />, repair: <PageRepair />, shopping: <PageShopping />, workers: <PageWorkers />, tools: <PageTools />, log: <PageLog /> }
  const pageKeys = NAV.map(n => n[0])

  const swipeHandlers = useSwipeable({
    onSwipedLeft: () => {
      const idx = pageKeys.indexOf(page)
      if (idx < pageKeys.length - 1) setPage(pageKeys[idx + 1])
    },
    onSwipedRight: () => {
      const idx = pageKeys.indexOf(page)
      if (idx > 0) setPage(pageKeys[idx - 1])
    },
