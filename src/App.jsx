...
// ─── Telegram — відправляється через GAS сервер (без CORS проблем) ──────────
...
function AppInner({ isAdmin, onLogout }) {
  ...
  const doWriteoff = () => {
    ...
    openConfirm(
      ...
      async () => {
        ...
        try {
          // Глобальне списання: матеріал один на складі — списуємо ОДИН РАЗ по назві
          // writeOff на сервері списує тільки з typeId-акума, тому синхронізуємо інші типи окремо
          const syncTasks = []
          const syncMatUpdates = []
          consumed.forEach(c => {
            if (c.fromStock <= 0) return
            const alreadyAdded = new Set()
            batteryTypes.forEach(bt => {
              const sameNamedMat = bt.materials.find(mx => mx.name === c.name)
              if (sameNamedMat && !alreadyAdded.has(c.name)) {
                // Для основного типу writeOff вже списує — пропускаємо
                if (bt.id !== type.id) {
                  syncTasks.push(api('updateMaterialStock', [bt.id, sameNamedMat.id, -c.fromStock]))
                }
                alreadyAdded.add(c.name)
    ...
    openConfirm('Видача на заготовку',
      ...
      async () => {
        ...
        try {
          ...
          // Синхронізуємо залишок по ВСІХ типах де є матеріал з такою назвою
          setBatteryTypes(prev => prev.map(t => ({
            ...t, materials: t.materials.map(m => m.name === mat.name ? { ...m, stock: Math.max(0, +(m.stock - qty).toFixed(4)) } : m)
          })))
  ...
  const doReturnPrep = async (prepId, all, customQty) => {
    ...
    try {
      ...
      // Синхронізуємо повернення по ВСІХ типах де є матеріал з такою назвою
      setBatteryTypes(prev => prev.map(t => ({
        ...t, materials: t.materials.map(m => m.name === item.matName ? { ...m, stock: +(m.stock + qty).toFixed(4) } : m)
      })))
  ...
  const doSubmitRepair = (repairEntry) => {
    ...
    openConfirm('Підтвердити ремонт',
      ...
      async () => {
        closeModal()
        try {
          // Ремонт: матеріал один на складі — списуємо ОДИН РАЗ по назві
          const syncTasks = []
          const syncMatUpdates = []
          repairEntry.materials.forEach(r => {
            if (!r.selected || !r.qty || r.qty <= 0) return
            const alreadyAdded = new Set()
            batteryTypes.forEach(bt => {
              const sameNamedMat = bt.materials.find(mx => mx.name === r.matName)
              if (sameNamedMat && !alreadyAdded.has(r.matName)) {
                // addRepair на сервері списує тільки з repairEntry.typeId — решту синхронізуємо
                if (bt.id !== repairEntry.typeId) {
                  syncTasks.push(api('updateMaterialStock', [bt.id, sameNamedMat.id, -r.qty]))
                }
                alreadyAdded.add(r.matName)
  ...
  const PageStock = () => {
    ...
    // Всі унікальні матеріали з усіх типів (з інших + з поточного для довідки)
    // Показуємо ВСІ — бо той самий матеріал може бути в різних типах з різною нормою
    ...
    batteryTypes.forEach(bt => {
      bt.materials.forEach(m => {
        ...
        if (!seen.has(key)) {
          seen.add(key)
          allUniqueMats.push(m)
        }
      })
    })
    const filteredSuggestions = allUniqueMats.filter(m =>
      !existingNames.has(m.name.toLowerCase()) // не показувати вже доданих
      && (!matSearch || m.name.toLowerCase().includes(matSearch.toLowerCase()))
    )

    const pickExisting = (m) => {
      // Підставляємо назву, одиницю і посилання — але perBattery і minStock залишаємо порожніми
      // бо для цього типу акумулятора норма може бути інша
      setNewMat(v => ({ ...v, name: m.name, unit: m.unit, shopUrl: m.shopUrl || '' }))
      setMatSearch(m.name)
      setMatPickerOpen(false)
    }
          ...
          {matPickerOpen && filteredSuggestions.length > 0 && (
            <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, background: G.card, border: `1px solid ${G.b2}`, borderRadius: 10, maxHeight: 220, overflowY: 'auto', marginTop: 4, boxShadow: '0 8px 24px #0008' }}>
              <div style={{ padding: '6px 10px', fontSize: 11, color: G.t2, borderBottom: `1px solid ${G.b1}` }}>
                Існуючі матеріали — назва і одиниця підставляться, норму вкажіть самі:
              </div>
              {filteredSuggestions.map(m => (
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: G.t2 }}>{m.unit}{m.shopUrl ? ' · є посилання' : ''}</div>
                  </div>
                  <span style={{ fontSize: 11, color: G.gn, marginLeft: 8, flexShrink: 0 }}>↵ вибрати</span>
                </div>
              ))}
              <div
